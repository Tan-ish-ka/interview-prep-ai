...
export function DashboardPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const targetUrls = url.trim();
      if (!targetUrls.startsWith('http://') && !targetUrls.startsWith('https://')) {
        throw new Error('Please enter a valid complete URL starting with http:// or https://');
      }

      const data = await fetchReport(targetUrls);
      setReport(data);
    } catch (err) {
